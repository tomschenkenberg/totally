...
async function callAPI(endpoint: string, method = "GET", body = null) {
    ...
    try {
        const response = await fetch(`/api/players/${uniqueCode}${endpoint}`, options)
        if (!response.ok) {
        ...
        persist(
            (set, get) => ({
                ...
                getPlayersSortedByScore: () => {
                    const playersArray = Object.entries(get().players).map(([id, player]) => ({
                        id: Number(id),
                        player,
                ...
                getTotalScore: (id) => {
                    ...
                    return player
                        ? Object.values(player.scores).reduce((total, score) => total + score, 0)
                        : 0
                },
